...
const active = ref(route.path === ROUTE_CONSTANTS.EVENTS ? 1 : 0)
...
const vueTabClick = (event: any) => {
  ...
  if (index === 1) {
    navigateTo(ROUTE_CONSTANTS.EVENTS)
  }
}
</script>
<template>
  <div v-if="!$route.params.slug" class="py-2">
    <TabView v-model:activeIndex="active" class="bg-transparent" @tab-change="vueTabClick">
      <TabPanel v-for="tab in DASHBOARD_TAB" :key="tab.id" :header="tab.label">
        <slot />
      </TabPanel>
    </TabView>
  </div>
