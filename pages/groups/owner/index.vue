...
watch(() => route.query, (newValue) => {
  ...
  if (queryParam.value?.q) {
    const decrypt = getDataFromQueryParams(queryParam.value?.q)
...
<template>
  <div class="flex flex-col gap-4 ">
    ...
    <template v-else>
      <TemplatesGroups :users="GroupData.rows" :heading="STRING_DATA.YOUR_GROUPS" :buttonLabel="STRING_DATA.CREATE_GROUP" v-on:card-click="handleCardClick"/>
      <NxPagination :totalCount="totalPage?.toString()" :currentPage="(pageRef+1)?.toString()" v-on:prev="prevPage" v-on:next="nextPage" />
    </template>
  </div>
</template>
